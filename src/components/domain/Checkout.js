...
export default function Checkout({ planner, tripMode="oneway", schedule, returnSchedule=null, seatOutbound, seatReturn, onFinish }) {
  ...
  const submit = (e) => {
    ...
    if (!isEmail(buyer.email)) newErrs.push({ id: "email", message: "Introduce un correo electrónico válido" });
    ...
    const tOut = {
      ...
      groupId,
      leg: "outbound",
      buyer: { nombre: buyer.nombre, email: buyer.email }, // <- clave alineada con el resto de la app
      planner,
      schedule,
    ...
    if (tripMode === "round" && returnSchedule) {
      const tRet = {
        ...
        groupId,
        leg: "return",
        buyer: { nombre: buyer.nombre, email: buyer.email }, // <- idem
        planner,
    ...
    <section aria-labelledby="checkout-title" className="checkout">
      ...
      <form onSubmit={submit} noValidate>
        ...
        <Field
          ...
          help="Recibirás los billetes y el localizador de cada uno."
        >
          <TextInput id="email" type="email" value={buyer.email} onChange={(e)=>setBuyer(b=>({...b, email: e.target.value}))} />
        </Field>

        {/* Nota: App asigna id="seatOutbound" y "seatReturn" en los SeatSelector para que ErrorSummary pueda enfocar directamente. */}
